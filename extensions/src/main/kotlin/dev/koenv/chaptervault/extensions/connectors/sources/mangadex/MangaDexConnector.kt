...
class MangaDexConnector(
    ...
    override suspend fun download(chapter: Chapter, format: ChapterFormat): Result<DownloadResult> {
        return when (val r = resolveAtHome(chapter.externalId)) {
            ...
            is Result.Success -> {
                ...
                Result.Success(
                    DownloadResult(
                        pages = cached.filenames.mapIndexed { i, filename ->
                            DownloadPage(
    ...
    private fun MangaDexMangaData.toSearchResult() = SeriesSearchResult(
        externalId = id,
    ...
    private fun MangaDexMangaData.toMetadata() = SeriesMetadata(
        externalId = id,
        title = attributes.preferredTitle(),
        coverUrl = relationships.coverUrl(id),
        description = attributes.preferredDescription(),
    ...
    private fun MangaDexChapterData.toChapterMetadata() = ChapterMetadata(
        externalId = id,
    ...
    private fun MangaDexMangaAttributes.preferredTitle(): String =
        title["en"] ?: title.values.firstOrNull() ?: ""
    ...
    private fun List<MangaDexRelationship>.coverUrl(mangaId: String): String? {
        val fileName = firstOrNull { it.type == "cover_art" }
            ?.attributes
