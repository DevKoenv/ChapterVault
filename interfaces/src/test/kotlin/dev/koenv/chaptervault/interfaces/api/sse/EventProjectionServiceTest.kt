...
class EventProjectionServiceTest {
    ...
    private suspend fun CoroutineScope.collectOne(
    ...
    @Test
    fun `TaskEnqueued emits task state_changed with QUEUED status`() =
        runBlocking {
    ...
    @Test
    fun `TaskStarted emits task state_changed with RUNNING status`() =
        runBlocking {
            val eventBus = InMemoryEventBus()
            val service = EventProjectionService(eventBus)
            service.start()
    ...
    @Test
    fun `TaskCompleted emits task state_changed with COMPLETED status`() =
        runBlocking {
            val eventBus = InMemoryEventBus()
            val service = EventProjectionService(eventBus)
            service.start()
    ...
    @Test
    fun `TaskFailed emits task state_changed with FAILED status and error message`() =
        runBlocking {
            val eventBus = InMemoryEventBus()
            val service = EventProjectionService(eventBus)
            service.start()
    ...
    @Test
    fun `TaskCancelled emits task state_changed with CANCELLED status`() =
        runBlocking {
            val eventBus = InMemoryEventBus()
            val service = EventProjectionService(eventBus)
            service.start()
    ...
    @Test
    fun `DownloadStatusChanged emits chapter download_status event`() =
        runBlocking {
            val eventBus = InMemoryEventBus()
            val service = EventProjectionService(eventBus)
            service.start()
