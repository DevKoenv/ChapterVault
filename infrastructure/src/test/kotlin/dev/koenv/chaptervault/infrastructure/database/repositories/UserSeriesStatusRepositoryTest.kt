...
class UserSeriesStatusRepositoryTest {
    ...
    @Test
    fun `setStatus stores status as string name`() =
        runBlocking {
    ...
    @Test
    fun `setStatus is idempotent — second call updates existing row`() =
        runBlocking {
            val userId = Id.generate()
            val seriesId = Id.generate()
    ...
    @Test
    fun `clearStatus removes the row`() =
        runBlocking {
            val userId = Id.generate()
            val seriesId = Id.generate()
    ...
    @Test
    fun `getStatus returns null when no status set`() =
        runBlocking {
            assertNull(repo.getStatus(Id.generate(), Id.generate()))
        }

    @Test
    fun `status is isolated per user`() =
        runBlocking {
