...
class ProgressRepository : ProgressApi {
    ...
    override suspend fun markRead(userId: Id, chapterId: Id): Result<Unit> = dbQuery {
        val u = userId.toString()
        val c = chapterId.toString()
        val exists = ProgressTable.selectAll()
    ...
    override suspend fun markUnread(userId: Id, chapterId: Id): Result<Unit> = dbQuery {
    ...
    override suspend fun getProgress(userId: Id, seriesId: Id): Result<ReadProgress> = dbQuery {
        val totalCount = ChapterTable.selectAll()
            .where { ChapterTable.seriesId eq seriesId.toString() }
            .count().toInt()

        val readCount = ProgressTable
