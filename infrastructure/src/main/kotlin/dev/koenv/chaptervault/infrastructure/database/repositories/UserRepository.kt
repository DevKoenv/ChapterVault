...
class UserRepository : AuthApi {
    ...
    override suspend fun register(credentials: Credentials, role: Role): Result<UserPrincipal> = dbQuery {
    ...
    override suspend fun authenticate(credentials: Credentials): Result<Pair<UserPrincipal, Session>> = dbQuery {
    ...
    override suspend fun invalidateSession(token: String): Result<Unit> = dbQuery {
        SessionTable.deleteWhere { SessionTable.token eq token }
        Result.Success(Unit)
    }
    ...
    private fun ResultRow.toPrincipal(): UserPrincipal {
        val roles = this[UserTable.roles].split(",")
