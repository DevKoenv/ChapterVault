...
class RateLimiterTest {
    @Test
    fun `tryAcquire returns 0 when under limit`() =
        runBlocking {
    ...
    @Test
    fun `tryAcquire returns positive ms when burst exhausted`() =
        runBlocking {
    ...
    @Test
    fun `tryAcquire does not record the attempt when rejected`() =
        runBlocking {
